...
impl Passport {
    ...
    fn is_valid2(&self) -> bool {
        self.byr.is_some() && self.byr.unwrap() >= 1920 && self.byr.unwrap() <= 2002
            ...
            && self.hgt.is_some() && self._check_hgt()
            && self.hcl.is_some() && Regex::new(r"^#[0-9a-f]{6}$").unwrap().is_match(self.hcl.as_ref().unwrap())
            && self.ecl.is_some() && Regex::new(r"(amb|blu|brn|gry|grn|hzl|oth)$").unwrap().is_match(self.ecl.as_ref().unwrap())
            && self.pid.is_some() && Regex::new(r"^[0-9]{9}$").unwrap().is_match(self.pid.as_ref().unwrap())
    }

    fn _check_hgt(&self) -> bool {
        let captures = Regex::new(r"^(\d+)(cm|in)$").unwrap().captures(self.hgt.as_ref().unwrap());
        if captures.is_none() {
            return false
        }
